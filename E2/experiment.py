...
import itertools
from psychopy import core, data, event, gui, logging, prefs, visual
...
# Version Numbers:
# 1.0: Initial
# 1.1: Improved tone preloading

# Set constants
experiment_name = 'ITMSR'  # Experiment name
...
registers = ['L', 'U']  # List of valid registers (used for checking manual entry in dialogue box)
pitches = [0, 1, 2, 3, 4, 5]  # Pitch levels (from 0-5)
iois = [400, 600]  # IOI conditions
conditions = [c for c in itertools.product(pitches, iois)]
practice_ioi = 500  # IOI of practice trials
repetitions_per_block = 2  # Number of repetitions of each pitch-IOI pair per block
blocks = 5  # Number of blocks
trials_per_block = len(conditions) * repetitions_per_block  # Number of trials per block

# Randomize trial order
trial_order = []
# Add practice trials (1 per pitch at 500 ms IOI)
practice_pitches = np.array(pitches)
np.random.shuffle(practice_pitches)
for pitch in practice_pitches:
    trial_order.append({'pitch': pitch, 'ioi': practice_ioi, 'event': 'practice'})
# Add main trials
for block in range(blocks):
    block_trials = np.array(conditions * repetitions_per_block)
    np.random.shuffle(block_trials)
    for trial in block_trials:
        trial_order.append({'pitch': trial[0], 'ioi': trial[1], 'event': 'trial'})
...
# Experimenter manually enters the register based on a predetermined randomization provided on the audiogram table
register = info_dict['register']
if register not in registers:
    raise ValueError('Invalid register entered! Must be one of %s.' % registers)
...
spr_trial = data.TrialHandler([{}], 1, method='sequential', 
                           dataTypes=['event', 'tap_types', 'tap_times', 'release_times',
                                      'peak_pressures', 'peak_times'])
trials = data.TrialHandler(trial_order, 1, method='sequential', 
                           dataTypes=['event', 'pitch', 'ioi', 'tone_times',
...
for trial in spr_trial:
    ...
    while len(release_times) < nspr_taps:
        if arduino_ser.in_waiting > 0:
            ...
            if in_byte == b'T':
                ...
                pressure_byte = arduino_ser.read()
                peak_pressures.append(int.from_bytes(pressure_byte, 'little', signed=False))
                peak_times.append(read_timestamp(arduino_ser))
    ...
    spr_trial.addData('peak_pressures', peak_pressures)
    spr_trial.addData('peak_times', peak_times)
    exp.nextEntry()
...
for trial in trials:
    ...
    # Set pitch and IOI for this trial
    pitch = trial.pitch
    ioi = trial.ioi
    ...
    # The message includes B to signal trial beginning, one character indicating the register,
    # a one-digit integer indicating the pitch, and a three-digit integer indicating the IOI.
    # The Arduino is programmed to look for these values after receiving a B.
    trial_message = b'BL%i%i' % (pitch, ioi) if register == 'L' else b'BU%i%i' % ( pitch, ioi)
    arduino_ser.write(trial_message)
    phase = 'B'
    ...
    trials.addData('peak_pressures', peak_pressures)
    trials.addData('peak_times', peak_times)
    exp.nextEntry()
    ...
    if block_number == 0 and trial_number == len(pitches):
